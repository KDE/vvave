...
MediaPlayer2Player::MediaPlayer2Player(Playlist * playListControler,
									   Player *audioPlayer, bool showProgressOnTaskBar, QObject* parent)
	: QDBusAbstractAdaptor(parent), m_playListControler(playListControler), m_audioPlayer(audioPlayer),
	  mProgressIndicatorSignal(QDBusMessage::createSignal(QStringLiteral("/org/maui/vvave"),
	  ...
	  mShowProgressOnTaskBar(showProgressOnTaskBar)
{
	...
	connect(m_playListControler, &Playlist::currentIndexChanged,
			this, &MediaPlayer2Player::playerSourceChanged, Qt::QueuedConnection);
	connect(m_playListControler, &Playlist::canPlayChanged,
